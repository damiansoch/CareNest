...
export function MedicationTracker({
  ...
  const grouped = TIME_ORDER.reduce<Record<TimeOfDay, Medication[]>>(
    (acc, tod) => {
      acc[tod] = medications.filter(
        (m) =>
          m.is_active &&
          m.schedules.some((s) => s.time_of_day === tod)
      );
      return acc;
    },
  ...
  return (
    <div className="tracker-page font-sans">
      {/* Header */}
      <div className="tracker-header border-b-2 border-black pb-4 mb-6">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="tracker-title text-3xl font-bold text-black">
              {t("title")}
            </h1>
            <p className="tracker-senior text-2xl font-semibold text-black mt-1">
              {senior.full_name}
            </p>
          </div>
          <div className="text-right">
            <p className="tracker-date text-xl font-medium text-black">
              {format(date, "d MMMM yyyy", { locale: dateLocale })}
            </p>
            <p className="text-sm text-gray-600 mt-1">CareNest</p>
          </div>
        </div>
      </div>

      {!hasAny ? (
        <p className="text-xl text-gray-600">{t("noMedications")}</p>
      ) : (
        <div className="tracker-sections space-y-6">
          {TIME_ORDER.map((tod) => {
      ...
      {/* Notes section */}
      <div className="tracker-notes mt-8 pt-4 border-t-2 border-black">
        <h3 className="text-xl font-bold text-black mb-3">{t("notes")}</h3>
        <div className="notes-lines">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="border-b border-gray-400 h-10 mb-1" />
          ))}
        </div>
      </div>
      ...
      <div className="border-2 border-gray-300 rounded-lg overflow-hidden">
        {medications.map((med, idx) => {
          ...
          return (
            ...
            >
              {/* Large checkbox */}
              <div className="flex-shrink-0 w-8 h-8 border-2 border-black rounded" />
              ...
              {/* Instructions */}
              {med.instructions && (
                <p className="text-sm text-gray-600 italic max-w-48 text-right hidden md:block">
                  {med.instructions}
                </p>
              )}
              ...
              {schedule?.custom_time && (
                <p className="flex-shrink-0 text-base font-medium text-black">
